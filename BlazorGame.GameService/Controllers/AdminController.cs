...
public class AdminController : ControllerBase
{
    ...
    public async Task<IActionResult> ExportPlayersToCsv()
    {
        ...
        csv.AppendLine("PlayerId,Health,Strength,Armor,Damage,HeartNumber,IsActive,CreatedAt");

        foreach (var player in players)
        {
            csv.AppendLine($"{player.CharacterId},{player.Health},{player.Strength},{player.Armor},{player.Damage},{player.HeartNumber},{player.IsActive},{player.CreatedAt:yyyy-MM-dd HH:mm:ss}");
        }

        return File(System.Text.Encoding.UTF8.GetBytes(csv.ToString()), "text/csv", "players.csv");
    }

    /// <summary>
    /// Exporte la liste des joueurs au format JSON.
    /// </summary>
    ...
    /// <summary>
    /// Exporte le classement au format CSV.
    /// </summary>
    ...
    public async Task<IActionResult> ExportLeaderboardToCsv()
    {
        ...
        var csv = new System.Text.StringBuilder();
        csv.AppendLine("Rank,PlayerId,TotalScore,GamesPlayed,Victories,Defeats,BestScore,AverageScore");

        int rank = 1;
        foreach (var entry in leaderboard)
        {
            csv.AppendLine($"{rank},{entry.PlayerId},{entry.TotalScore},{entry.GamesPlayed},{entry.Victories},{entry.Defeats},{entry.BestScore},{entry.AverageScore:F2}");
            rank++;
        }

        return File(System.Text.Encoding.UTF8.GetBytes(csv.ToString()), "text/csv", "leaderboard.csv");
    }
}
